...
struct PublishActivityView: View {
  ...
  var body: some View {
    ...
    .customNavigationBar(
      title: viewModel.step == .selectProduct ? "选择产品" : "发布活动", router: router,
      leadingItem: {
        Button(
          action: {
      ...
      VStack {
        ...
        HStack {
          Text("审核")
            .foregroundColor(Color(hex: "#333333"))
          Spacer()
          Toggle("", isOn: $viewModel.isAudit)
            .labelsHidden()
            .tint(.black)
            .scaleEffect(0.8, anchor: .trailing)
        }

        if !viewModel.isAudit && viewModel.rewardType != .selfManaged {
          // 是否自动发放奖励
          HStack {
            Text("自动发放奖励")
              .foregroundColor(Color(hex: "#333333"))
            Spacer()
            Toggle("", isOn: $viewModel.isAutoReward)
              .labelsHidden()
              .tint(.black)
              .scaleEffect(0.8, anchor: .trailing)
          }
          ...
          // tips
          HStack {
            Text("自动发放奖励，用户报名即可获得奖励, 多种奖励时候随机发放")
              .font(.system(size: 12))
              .foregroundColor(Color(hex: "#666666"))
            Spacer()
          }

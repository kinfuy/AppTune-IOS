        ...
        if !activeService.pendingActiveReviews.isEmpty {
          VStack(alignment: .leading, spacing: 12) {
            ...
            ForEach(activeService.pendingActiveReviews) { activity in
              ActivityReviewCard(activity: activity) {
                // TODO: 处理审核操作
              }
            }
          }
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      HStack(spacing: 12) {
        ...
        Link(destination: URL(string: product.link) ?? URL(string: "https://www.apple.com")!) {
...
struct ActivityReviewCard: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 12) {
      // 封面图
      AsyncImage(url: URL(string: activity.cover)) { image in
        image
          .resizable()
          .scaledToFill()
      } placeholder: {
